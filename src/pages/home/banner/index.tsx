...
import { homeManageApi, type BannerItem } from '@/api/homeManage'
...
function BannerList() {
  ...
  const loadData = async (page = 1, pageSize = 10) => {
    ...
    try {
      ...
      const res = await homeManageApi.getBannerList({
        pageNum: page,
        pageSize,
        ...values,
      })
  ...
  const columns: ColumnsType<BannerItem> = [
    ...
    },
    {
      ...
      width: 120,
      render: (image: string) => (
        <Image width={80} height={40} src={image} />
      ),
    },
    {
      title: '标题',
  ...
  const handleToggleStatus = (record: BannerItem) => {
    const newStatus = record.status === 1 ? 0 : 1
    Modal.confirm({
      title: newStatus === 1 ? '确认启用' : '确认禁用',
      content: `确定要${newStatus === 1 ? '启用' : '禁用'}该Banner吗？`,
      onOk: async () => {
        try {
          await homeManageApi.updateBanner({ ...record, status: newStatus })
  ...
  const handleDelete = (record: BannerItem) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await homeManageApi.deleteBanner(record.id)
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editingItem) {
        await homeManageApi.updateBanner({ ...editingItem, ...values })
        message.success('编辑成功')
      } else {
        await homeManageApi.addBanner(values)
  ...
  return (
    <div>
      ...
      <Modal
        title={editingItem ? '编辑Banner' : '新增Banner'}
        open={modalVisible}
